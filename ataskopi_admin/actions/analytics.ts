...
export async function getNetProfitAnalytics(outletId: string, startDate: Date, endDate: Date) {
    ...
    const grossRevenue = revenues.reduce((sum, rev) => sum + Number(Number(rev.grossRevenue) > 0 ? rev.grossRevenue : rev.cashAmount), 0)
...
export async function getMonthlyProfitSummary(outletId: string, months: number = 6) {
    ...
    for (let i = months - 1; i >= 0; i--) {
        ...
        const grossRevenue = revenues.reduce((sum, r) => sum + Number(Number(r.grossRevenue) > 0 ? r.grossRevenue : r.cashAmount), 0)
...
export async function getDailyProfitTrend(outletId: string, startDate: Date, endDate: Date) {
    ...
    revenues.forEach(r => {
        ...
        const amt = Number(Number(r.grossRevenue) > 0 ? r.grossRevenue : r.cashAmount)
...
export async function getAssetsROI(outletId: string) {
    ...
    const roiData = assets.map(asset => {
        const revSince = revenues.filter(r => r.date >= asset.purchaseDate).reduce((s, r) => s + Number(Number(r.grossRevenue) > 0 ? r.grossRevenue : r.cashAmount), 0)
